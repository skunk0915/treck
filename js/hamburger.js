document.addEventListener('DOMContentLoaded', function () {
    ...
    function renderRelated() {
        console.log('[renderRelated] Starting...');
        console.log('[renderRelated] relatedArticlesData:', relatedArticlesData);
        console.log('[renderRelated] relatedListEl:', relatedListEl);
        console.log('[renderRelated] relatedSection:', relatedSection);

        relatedListEl.innerHTML = ''; // Clear container
        if (typeof relatedArticlesData !== 'undefined' && Object.keys(relatedArticlesData).length > 0) {
            console.log('[renderRelated] relatedArticlesData is defined with keys:', Object.keys(relatedArticlesData));
            let hasRelated = false;

            // Iterate object keys (tags)
            for (const [tag, articles] of Object.entries(relatedArticlesData)) {
                console.log('[renderRelated] Processing tag:', tag, 'with', articles.length, 'articles');
                if (articles && articles.length > 0) {
                    ...
                    console.log('[renderRelated] Appended tagContainer to relatedListEl');
                }
            }

            console.log('[renderRelated] hasRelated:', hasRelated);
            relatedSection.style.display = hasRelated ? 'block' : 'none';
            console.log('[renderRelated] relatedSection.style.display set to:', relatedSection.style.display);
        } else {
            console.log('[renderRelated] relatedArticlesData is undefined or empty');
