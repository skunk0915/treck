...
sort($files); // Ensure deterministic order
$articles = [];
$allTagsCounter = [];
...
// Sort articles by filename to ensure deterministic processing order
...
?>
    <main class="container">
        <article class="post">
            ...
            <section id="main-all-section" class="menu-section-static" style="margin-top: 3rem; border-top: 1px solid #ddd; padding-top: 2rem;">
                 <h3 style="color: var(--primary-color); font-size: 1.2rem; margin-bottom: 1rem; border-bottom: 2px solid #eee; padding-bottom: 0.5rem;">こんな記事もあります</h3>
                 <div id="main-all-list" class="menu-list"></div>
            </section>
        </article>
    </main>
...
shuffle($articles);
$relatedByTag = []; // Reset related articles for homepage
...
$jsonArticles = [];
// Re-sort articles for JSON to be deterministic (since we shuffled for homepage)
usort($articles, function ($a, $b) {
    return strcmp($a['filename'], $b['filename']);
});
...
function atomic_put_contents($path, $content)
{
    if (file_exists($path)) {
        $existing = file_get_contents($path);
        if ($existing === $content) {
            // echo "Skipping unchanged file: " . basename($path) . "\n";
            return;
        }
    }
...
function getArticleMetadataBuild($filename, $articleDir, $articleMetaManager, $articleTagsMap)
{
    ...
    sort($tags); // Ensure deterministic tag order
